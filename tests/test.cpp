...
TEST(ParserTest, GetCInstructionCompString) {
  ...
  EXPECT_STREQ(std::get<std::string>(get_comp_string("AM=3+A;JMP")).c_str(),
               "3+A");

  EXPECT_STREQ(std::get<std::string>(get_comp_string("AM=3+AD")).c_str(),
               "3+AD");

  EXPECT_STREQ(std::get<std::string>(get_comp_string("AM=3+A;")).c_str(),
               "3+A");
}

TEST(ParserTest, GetCInstructionJMPString) {

  EXPECT_STREQ(std::get<std::string>(get_jmp_string("AM=3+A;JMP")).c_str(),
               "JMP");
  EXPECT_EQ(std::get<assembler_error>(get_jmp_string("AM=3+A")).ec,
...
TEST(ParserTest, GetCInstructionCompBincode) {
  ...
  comp_bincode bin =
      std::get<comp_bincode>(parse_string_into_comp_bincode("0"));
